    ...
    <div className="space-y-12 pb-32 animate-fade-in bg-background min-h-screen">
      <motion.section variants={itemVariants} className="text-center pt-8">
            <button 
                ...
                className="flex items-center gap-3 px-8 py-4 bg-white text-zinc-950 rounded-3xl font-black mx-auto hover:bg-slate-100 transition-all shadow-2xl shadow-black/20 active:scale-95 group"
            >
                <History className="h-5 w-5 text-emerald-500 group-hover:rotate-12 transition-transform" />
      ...
      <motion.section variants={itemVariants} className="mb-14">
        <div className="flex items-center justify-between mb-8 px-2">
          ...
          <div className="h-px flex-1 bg-slate-800 ms-6"></div>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {(getDailyTasks(progress?.totalMemorized || 0, progress?.currentPage || 1) || []).map((task) => (
            <div key={task.id} className="bg-card/40 border border-white/5 rounded-3xl p-8 hover:shadow-2xl hover:border-amber-500/30 transition-all duration-300 group shadow-xl shadow-black/40 relative overflow-hidden">
              ...
              <div className="relative flex items-center gap-5 mb-6">
                <div className="p-4 bg-amber-500/10 rounded-2xl text-amber-400 group-hover:bg-amber-500/20 transition-all">
                  {task.id === 'new_hifz' ? <BookOpen className="h-6 w-6" /> : task.id === 'intensive_review' ? <Flame className="h-6 w-6" /> : <Calendar className="h-6 w-6" />}
                </div>
                <div>
                  <h4 className="font-black text-base text-foreground uppercase tracking-tight">{t(`tasks.${task.id}`)}</h4>
                  <span className="text-[10px] font-black text-amber-400 uppercase tracking-[0.3em] leading-none block mt-1.5">
                    {task.pages} {t('dashboard.pages')}
                  </span>
                </div>
              </div>
              <p className="relative text-sm text-slate-500 leading-relaxed font-bold opacity-80">{task.desc}</p>
            </div>
          ))}
        </div>
      </motion.section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {reviewTools.map((tool) => (
                <Link 
                    key={tool.title} 
                    to={tool.path}
                    className="flex flex-col items-center justify-center p-12 bg-card/40 border border-white/10 rounded-3xl shadow-xl shadow-black/40 hover:scale-[1.03] hover:shadow-2xl transition-all text-center group relative overflow-hidden"
                >
                    ...
                    <span className="font-black text-lg text-foreground group-hover:text-emerald-400 transition-colors uppercase tracking-widest">
                        {tool.title}
                    </span>
                </Link>
            ))}
