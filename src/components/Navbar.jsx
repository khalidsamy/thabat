...
const Navbar = () => {
  ...
  return (
    <nav className="bg-card/60 backdrop-blur-md border-b border-white/5 transition-all duration-500 sticky top-0 z-50 shadow-sm shadow-black/20">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden lg:flex items-center gap-2 mx-8 bg-white/5 p-1.5 rounded-3xl border border-white/5">
            {menuItems.slice(0, 3).map((item) => (
              <NavLink
                ...
                className={({ isActive }) => `flex items-center gap-3 px-6 py-3 rounded-2xl text-sm font-black transition-all duration-500 uppercase tracking-widest ${
                  isActive 
                    ? `bg-zinc-950 shadow-lg shadow-black/20 ${item.color} scale-100` 
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-3 px-4 py-2.5 rounded-2xl text-xs font-black text-zinc-500 hover:bg-white/5 transition-all uppercase tracking-widest"
            ...
            <Link 
              ...
              className="hidden md:flex items-center gap-3 pl-2 pr-4 py-1.5 bg-white/5 rounded-2xl border border-white/5 hover:border-emerald-500/30 transition-all group"
            >
              <div className="w-9 h-9 bg-white rounded-xl flex items-center justify-center shadow-lg shadow-black/10">
                <span className="text-zinc-950 font-black text-sm uppercase">
        ...
        {isMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className={`fixed inset-y-0 ${i18n.language === 'ar' ? 'right-0' : 'left-0'} w-80 z-[101] bg-card/95 backdrop-blur-3xl border-x border-white/10 p-8 lg:hidden shadow-2xl flex flex-col h-screen`}
            >
              <div className="flex items-center justify-between mb-10">
                 ...
                 <button onClick={() => setIsMenuOpen(false)} className="p-2.5 bg-white/5 rounded-xl text-slate-400 hover:text-foreground transition-all">
                    <X className="h-6 w-6" />
                 </button>
              </div>

              <div className="flex-1 overflow-y-auto -mx-2 px-2 py-4 space-y-3">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 px-4 italic">Navigation</p>
                {menuItems.map((item) => (
                  <NavLink
                    key={item.path}
                    to={item.path}
