...
const RecoveryCard = ({ onStartSmall }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-card/40 border-2 border-rose-900/30 rounded-[3rem] p-6 lg:p-8 shadow-2xl shadow-rose-950/20 h-full max-h-[480px] overflow-hidden relative group backdrop-blur-md"
    >
      ...
      <div className="relative flex flex-col h-full items-center justify-between gap-6">
        <div className="flex flex-col items-center text-center gap-4">
          <div className="w-16 h-16 bg-rose-900/40 rounded-2xl flex items-center justify-center shrink-0 shadow-lg shadow-black/20 group-hover:scale-110 transition-transform">
            <HeartPulse className="h-8 w-8 text-rose-500 animate-pulse" />
          </div>

          <div className="space-y-1">
            <span className="text-[9px] font-black text-rose-400 uppercase tracking-[0.2em] bg-rose-950 px-3 py-1 rounded-full">
              {t('recovery.doctor_mode')}
            </span>
            <h3 className="text-xl font-black text-foreground">
              {t('recovery.doctor_title')}
            </h3>
            <p className="text-[11px] font-medium text-slate-500 max-w-[200px] mx-auto leading-tight">
              {t('recovery.doctor_subtitle')}
            </p>
          </div>
        </div>

        <div className="w-full bg-black/20 backdrop-blur-sm rounded-[2rem] p-5 border border-white/5 space-y-3">
          <p className="text-base font-bold text-foreground leading-snug line-clamp-3" dir="rtl">
             "{hadith.text}"
          </p>
          <div className="flex justify-between items-center border-t border-white/5 pt-3">
            <p className="text-[9px] font-black text-rose-400/60 uppercase tracking-widest" dir="rtl">
              — {hadith.source}
            </p>
            <p className="text-[9px] font-medium text-slate-500 line-clamp-1 max-w-[150px]">
              {hadith.translation}
            </p>
          </div>
        </div>
        ...
        <div className="w-full space-y-1.5">
           <div className="flex justify-between items-end">
              <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Surah Goal</span>
              <span className="text-[8px] font-black text-rose-500 uppercase">Recovery</span>
           </div>
           <div className="h-1.5 w-full bg-rose-900/20 rounded-full overflow-hidden">
              <motion.div 
                 initial={{ width: 0 }}
                 animate={{ width: '35%' }}
        ...
        <motion.button
          ...
          className="w-full bg-rose-500 hover:bg-rose-600 text-white py-4 rounded-2xl font-black text-xs shadow-xl shadow-rose-950/20 flex items-center justify-center gap-2 group transition-all"
        >
